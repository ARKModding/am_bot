...
class StarboardCog(commands.Cog):
    ...
    async def on_ready(self):
        ...
        channel: discord.TextChannel = self.bot.get_channel(STARBOARD_TEXT_CHANNEL_ID)
        ...
        async for message in channel.history(limit=None):
            ...
            if not message.embeds or not message.embeds[0].fields:
                logger.warning(f'Message in Starboard channel missing embeds: {message}')
                continue
            found = channel_id_pattern.findall(message.embeds[0].fields[0].value)
            if not found:
                logger.warning(f'Message in Starboard channel, unable to parse related channel id. Message: {message}')
                continue
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        logger.debug(f'Payload Received. Channel ID: {payload.channel_id}, Member ID: {payload.member.id}, '
        ...
        starboard_channel: discord.TextChannel = self.bot.get_channel(STARBOARD_TEXT_CHANNEL_ID)
        avatar_url = str(payload.member.avatar_url_as(static_format='png', size=128))
        embed = {
            'author': {
            ...
            'fields': [
                {
                    'name': '\u200b',
                    'value': f'**[Click to jump to message!]({message.jump_url})**',
                    'inline': False
                }
            ],
            'color': 3375061 if self._last_message.embeds[0].color == 16769024 else 16769024,
        ...
        self._last_message = await starboard_channel.send(embed=discord.Embed.from_dict(embed))
